...
@section('content')
<div class="bg-slate-50 dark:bg-slate-900 min-h-screen py-12 transition-colors duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-16">
            ...
            <h1 class="text-3xl md:text-5xl font-extrabold text-slate-900 dark:text-white mt-4 mb-4">
                Showcase <span class="text-blue-600 dark:text-blue-400">Produk Digital</span>
            </h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div onclick="openModal({{ json_encode($project) }})" 
                 class="group bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col h-full ring-1 ring-slate-900/5 dark:ring-white/5">
                
                <div class="h-52 w-full bg-slate-100 dark:bg-slate-700 relative overflow-hidden">
                    @if($project->image)
                    ...
                    <div class="absolute top-4 left-4">
                        <span class="px-3 py-1 bg-white/95 dark:bg-slate-900/90 backdrop-blur text-xs font-bold text-blue-700 dark:text-blue-400 rounded-lg shadow-sm tracking-wide uppercase border border-blue-100 dark:border-blue-900">
                            {{ $project->category }}
                        </span>
                    </div>
                </div>
                ...
                <div class="p-6 flex flex-col flex-grow">
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        @if(is_array($project->tech_stack) || is_object($project->tech_stack))
                            @foreach(array_slice((array)$project->tech_stack, 0, 3) as $stack)
                                <span class="text-[10px] font-bold px-2 py-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-md border border-slate-200 dark:border-slate-600">
                                    {{ $stack }}
                                </span>
                            @endforeach
                            @if(count((array)$project->tech_stack) > 3)
                                <span class="text-[10px] font-bold px-2 py-1 bg-slate-50 dark:bg-slate-700 text-slate-400 dark:text-slate-500 rounded-md border border-transparent dark:border-slate-600">+{{ count((array)$project->tech_stack) - 3 }}</span>
                            @endif
                        @endif
                    </div>

                    <p class="text-slate-600 dark:text-slate-400 text-sm line-clamp-2 mb-6 leading-relaxed">
                        {{ $project->description }}
                    </p>
                    ...
                    <div class="mt-auto pt-4 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between">
                        ...
                        <span class="text-blue-600 dark:text-blue-400 text-sm font-semibold group-hover:translate-x-1 transition-transform flex items-center gap-1">
...
<div id="projectModal" class="fixed inset-0 z-[60] hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div class="fixed inset-0 z-10 overflow-y-auto">
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            <div class="relative transform overflow-hidden rounded-2xl bg-white dark:bg-slate-900 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-2xl opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95 border border-slate-200 dark:border-slate-700" id="modalPanel">
                ...
                <div class="px-8 py-8">
                    ...
                    <div class="prose prose-slate dark:prose-invert prose-sm max-w-none mb-8">
                        <p id="modalDescription" class="leading-relaxed text-slate-600 dark:text-slate-300 text-base"></p>
                    </div>

                    <div class="bg-blue-50/50 dark:bg-slate-800/50 rounded-xl p-6 border border-blue-100 dark:border-slate-700 mb-8">
                        <div class="flex items-center gap-2 mb-4">
                            <div class="p-1.5 bg-blue-100 dark:bg-blue-900/30 rounded text-blue-600 dark:text-blue-400">
                                <i class="fa-solid fa-shield-halved"></i>
                            </div>
                            <h4 class="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wide">Akses Demo</h4>
                        </div>
                        
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm relative group">
                                <label class="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 block mb-1">Username / Email</label>
                                <div class="flex justify-between items-center">
                                    <code id="modalUser" class="text-slate-800 dark:text-slate-200 font-mono font-bold select-all text-sm">-</code>
                                    <button onclick="copyText('modalUser')" class="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                            ...
                            <div class="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm relative group">
                                <label class="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 block mb-1">Password</label>
                                <div class="flex justify-between items-center">
                                    <code id="modalPass" class="text-slate-800 dark:text-slate-200 font-mono font-bold select-all text-sm">-</code>
                                    <button onclick="copyText('modalPass')" class="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                                        <i class="fa-regular fa-copy"></i>
                                    </button>
                                </div>
                            </div>
                    ...
                    <a id="modalLink" href="#" target="_blank" class="flex items-center justify-center w-full py-3.5 px-4 rounded-xl shadow-lg shadow-blue-600/20 bg-blue-600 hover:bg-blue-700 text-white font-bold transition-all transform hover:-translate-y-0.5 gap-2">
...
<script>
    ...
    function openModal(data) {
        ...
        // Isi Tech Stack (Perhatikan penambahan class dark)
